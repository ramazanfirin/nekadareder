...
public class EntityService<E>  implements EntityDao<E> {
	...
	public List<E> findByProperty(String prop, Object val,int limit) throws Exception {
        return (List<E>) getEntityManager().createQuery("select x from " + getEntityClass().getSimpleName() + " x where x." + prop + " = ?1").setParameter(1, val).setMaxResults(limit).getResultList();
    }
